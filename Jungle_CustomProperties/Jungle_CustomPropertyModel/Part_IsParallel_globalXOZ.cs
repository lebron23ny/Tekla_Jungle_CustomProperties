...
namespace Jungle_CustomProperties.Jungle_CustomPropertyModel
{
    ...
    [ExportMetadata("CustomProperty", "CUSTOM.ME_IS_PARALLEL_GLOBAL_XOZ")]
    public class Part_IsParallel_globalXOZ : ICustomPropertyPlugin
    {
        ...
        public double GetDoubleProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is Part part)
                doubleProperty = ToolsModel.IsParallel_Plane_IN_GLOBAL(part, geometricPlane);
        ...
        public int GetIntegerProperty(int objectId)
        {
            int intProperty = -1;
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is Part part)
                intProperty = ToolsModel.IsParallel_Plane_IN_GLOBAL(part, geometricPlane);
            return intProperty;
        }
