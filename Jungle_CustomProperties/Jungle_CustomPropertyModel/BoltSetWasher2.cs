...
{ 
    ...
    [ExportMetadata("CustomProperty", "CUSTOM.ME_BOLT_SET_WASHER2")]
    public class BoltSetWasher2 : ICustomPropertyPlugin
    {
        public double GetDoubleProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is BoltGroup boltGroup)
                doubleProperty = Convert.ToDouble(ToolsModel.SetWasher2(boltGroup));
        ...
        public int GetIntegerProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is BoltGroup boltGroup)
                doubleProperty = Convert.ToInt32(ToolsModel.SetWasher2(boltGroup));
            return doubleProperty;
        }
        ...
        public string GetStringProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is BoltGroup boltGroup)
                doubleProperty = Convert.ToString(ToolsModel.SetWasher2(boltGroup));
            return doubleProperty;
        }
    }
}
