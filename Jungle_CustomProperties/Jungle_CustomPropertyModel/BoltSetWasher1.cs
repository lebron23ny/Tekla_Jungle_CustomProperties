...
namespace Jungle_CustomProperties.Jungle_CustomPropertyModel
{
    ...
    [ExportMetadata("CustomProperty", "CUSTOM.ME_BOLT_SET_WASHER1")]
    public class BoltSetWasher1 : ICustomPropertyPlugin
    {
        public double GetDoubleProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is BoltGroup boltGroup)
                doubleProperty = Convert.ToDouble(ToolsModel.SetWasher1(boltGroup));
        ...
        public int GetIntegerProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is BoltGroup boltGroup)
                doubleProperty = Convert.ToInt32(ToolsModel.SetWasher1(boltGroup));
            return doubleProperty;
        }
        ...
        public string GetStringProperty(int objectId)
        {
            ...
            if (new Tekla.Structures.Model.Model().SelectModelObject(new Identifier(objectId)) is BoltGroup boltGroup)
                doubleProperty = Convert.ToString(ToolsModel.SetWasher1(boltGroup));
            return doubleProperty;
        }
    }
}
